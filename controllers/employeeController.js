  ...
  db.query(checkEmployeeIdSql, [employeeId], (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        ...
        const contactNumberValue = contact_number || null;
        const emergencycontactnumberValue = emergency_contact_number || null;
        const hiredateValue = hire_date || null;
        const startdateValue = start_date || null;

        const sql = "INSERT INTO ETB_HR_EMPLOYEE_DETAILS1 (EMPLOYEE_ID, FIRST_NAME, LAST_NAME, GENDER, BIRTH_DATE, CONTACT_NUMBER, EMERGENCY_CONTACT_NUMBER, OFFICE_EMAIL, PERSONAL_EMAIL, CURRENT_ADDRESS, PERMANENT_ADDRESS, DEPARTMENT, JOB_POSITION, EMPLOYEE_RANK, JOB_LOCATION, HIRE_DATE, START_DATE, MANAGER_NAME, EMP_IMAGE, EMP_IMAGE_NAME, CREATED_BY, LAST_UPDATED_BY) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        const values = [
          employeeId,
          first_name,
          last_name,
